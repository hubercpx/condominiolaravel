...
<body>
  ...
  <button type="button" class="btn btn-outline-success"><a href='{{url("empleado/create")}}' style="text-decoration:none">Crear Nuevo</a></button>
    ...
    <form method="POST" action='{{ url("empleado/update/{$empleado->id}")}}'>
        <legend>Editar Empleado</legend>
        <div class="form-group">
          <label for="exampleInputEmail1">Usuario</label>
          <input type="text" class="form-control" aria-describedby="emailHelp" name="usuario" value="{{$empleado->usuario}}">
        </div>
        <div class="form-group">
          <label for="exampleInputPassword1">Contrasena</label>
          <input type="password" class="form-control" aria-describedby="emailHelp" name="contrasena" value="{{$empleado->contrasena}}">
        </div>
